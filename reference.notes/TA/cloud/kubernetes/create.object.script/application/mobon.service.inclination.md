...
kind: Deployment
metadata:
  name: mobon-service-inclination
  namespace: mobon
  labels:
    app: mobon-service-inclination
spec:
  selector:
    matchLabels:
      app: mobon-service-inclination
...
#            failureThreshold: 3
          env:
            ...
            - name: JAVA_OPTS
              value: "$(JAVA_OPTS) -Dobj_name=inclination-$(_POD_IP)"
            - name: JAVA_OPTS
          ...
          args:
            - source /etc/profile;
              ...
              echo "########################################";
              mkdir -p /pgms/mobon/inclination.service;
              cp -R /repository/git/mobon/inclination.git/* /pgms/mobon/inclination.service;
              echo 'org.gradle.jvmargs='$JAVA_OPTS >> /pgms/mobon/inclination.service/gradle.properties;
              gradle --build-file /pgms/mobon/inclination.service/build.gradle :mobon.service.inclination:bootRun -Dspring.profiles.active=prod -PjvmArgs="$(JAVA_OPTS)";
#              gradle --build-file /pgms/mobon/inclination.service/build.gradle :mobon.service.inclination:bootJar;
#              echo "########################################";
#              echo "run springboot application with scouter agent.java...";
#              echo "########################################";
#              java $JAVA_OPTS -Dspring.profiles.active=prod -jar /pgms/mobon/inclination.service/mobon.service.inclination/build/libs/mobon.service.inclination-1.0.jar;
#              tail -f /dev/null;
#          lifecycle:
#            postStart:
#              exec:
...
#              mountPath: /etc/git-secret
          env:
            - name: GIT_SYNC_REPO
              value: http://172.20.0.7:9000/enliple/mobon/service/inclination.git
...
kind: Service
metadata:
  name: mobon-service-inclination-svc
  namespace: mobon
  labels:
    app: mobon-service-inclination
spec:
